...
from app.models.health_record_model import HealthRecord
from app import db
from app.schemas.health_record_schema import HealthRecordSchema
...
class HealthRecordResource(Resource):
    def get(self, record_id=None):
        ...
        try:
            if record_id:
                ...
                if not record:
                    return {"error": "Health record not found"}, 404
                return HealthRecordSchema().dump(record), 200

            records = HealthRecord.query.all()
            return HealthRecordSchema(many=True).dump(records), 200
        except Exception as e:
            return {"error": str(e)}, 500
    ...
    def post(self):
        ...
        try:
            new_record = HealthRecord(
                type=args["type"], status=args["status"], notes=args.get("notes")
            )
            db.session.add(new_record)
            db.session.commit()
            return {"message": "Health record created successfully", "id": new_record.id}, 201
    ...
    def put(self, health_record_id):
        ...
        try:
            record = HealthRecord.query.get(health_record_id)
            if not record:
                return {"error": "Health record not found"}, 404
            ...
            return {"error": str(e)}, 500
    def patch(self, health_record_id):
        """Update a health record (partial update)."""
        parser = reqparse.RequestParser()
        parser.add_argument("type", required=False)
        parser.add_argument("status", required=False)
        parser.add_argument("notes", required=False)
        ...
        try:
            ...
            db.session.commit()
            return {"message": "Health record partially updated successfully"}, 200
        except Exception as e:
            return {"error": str(e)}, 500
