...
def login():
    try:
        ...
        identity = str(user.id)  # Use only the user ID as a string
...
def get_user_profile():
    try:
        current_user = get_jwt_identity()  # Get full identity dictionary
        user_id = current_user["id"]  # Access 'id' field
        role = current_user["role"]

        # Fetch user profile based on role
        if role == "Provider" or role == "Admin":
